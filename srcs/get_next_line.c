...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/06 15:52:44 by kquerel           #+#    #+#             */
/*   Updated: 2022/12/24 18:23:22 by kquerel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_sub_or_join(char *stash, char *buffer)
{
	...
	if (stash == NULL)
		new_stash = ft_substr(buffer, 0, BUFFER_SIZE);
	else
...
char	*get_next_line(int fd)
{
	...
	while (ft_strchr(stash, '\n') == 0 && ret_read != 0)
	{
		...
		stash = ft_sub_or_join(stash, buffer_or_return);
	}
	free (buffer_or_return);
	buffer_or_return = ft_write_one_line(stash);
	stash = ft_reinit_stash(stash);
